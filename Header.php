<?php
/*
#*##*##*##*##*##*##*##*##*##*##*##*##*# 
#*#                                 #*#               
#*#  Coded by atack irani hacker    #*#
#*#                                 #*#
#*#  mail: king.hanter129@gmail.com #*#
#*#                                 #*#
#*#         MaDe In iran            #*#
#*#                                 #*#
#*# Note: Don't change my copyright #*#
#*#          please , ok ?          #*#
#*#                                 #*#
#*#                                 #*#
#*##*##*##*##*##*##*##*##*##*##*##*##*#
*/

$uname = php_uname();
if(preg_match("/Linux/",$uname)){
// if OS = linux do this command    
    system("clear");
    } else {
// else do this command 
        system("cls");
        }
$version = "v2.0";
$white = "\e[97m";
$black = "\e[30m\e[1m";
$yellow = "\e[93m";
$orange = "\e[38;5;208m";
$blue   = "\e[34m";
$lblue  = "\e[36m";
$cln    = "\e[0;94m";
$green  = "\e[92m";
$fgreen = "\e[32m";
$red    = "\e[91m";
$magenta = "\e[35m";
$bluebg = "\e[44m";
$lbluebg = "\e[106m";
$greenbg = "\e[42m";
$lgreenbg = "\e[102m";
$yellowbg = "\e[43m";
$lyellowbg = "\e[103m";
$redbg = "\e[101m";
$grey = "\e[37m";
$cyan = "\e[36m";
$bold   = "\e[1m";
$nbold = "\e[1;97m";
$end = $bold.$orange."-==============================================================================-"."\n";
echo $red.$bold."                                 __________"."\n";
echo "                              .~#########%%;~."."\n";
echo "                             /############%%;`\ "."\n";
echo "                            /######/~\/~\%%;,;,\ "."\n";
echo "                           |#######\    /;;;;.,.|"."\n";
echo "                           |#########\/%;;;;;.,.|"."\n";
echo "                  XX       |##/~~\####%;;;/~~\;,|       XX"."\n";
echo "                XX..X      |#|  o  \##%;/  o  |.|      X..XX"."\n";
echo $white."              XX.....X     |##\____/##%;\____/.,|     X.....XX"."\n";
echo "         XXXXX.....XX      \#########/\;;;;;;,, /      XX.....XXXXX"."\n";
echo "        X |......XX%,.@      \######/%;\;;;;, /      @#%,XX......| X"."\n";
echo "        X |.....X  @#%,.@     |######%%;;;;,.|     @#%,.@  X.....| X"."\n";
echo "        X  \...X     @#%,.@   |# # # % ; ; ;,|   @#%,.@     X.../  X"."\n";
echo "         X# \.X        @#%,.@                  @#%,.@        X./  #"."\n";
echo $black."          ##  X          @#%,.@              @#%,.@          X   #"."\n";
echo "        , '# #X            @#%,.@          @#%,.@            X ##"."\n";
echo "           `###X             @#%,.@      @#%,.@             ####'"."\n";
echo "          . ' ###              @#%.,@  @#%,.@              ###`'"."\n";
echo "            . ';'                @#%.@#%,.@                ;'` ' ."."\n";
echo "              '                    @#%,.@                   ,."."\n";
echo $yellow.$bold."                         -============================-                "."\n";
echo $yellow.$bold."                          | Name   ".$green."|".$red." Bad bot $version    ".$yellow."|                "."\n";
echo $yellow.$bold."                          | Author ".$green."|".$blue." KING-HANTER   ".$yellow."  |                "."\n";
echo $yellow.$bold."                          | From   ".$green."|".$white." IRAN            ".$yellow."|                "."\n";
echo $yellow.$bold."                         -============================-                "."\n";
echo $yellow.$bold."\n                       script will be started pls wait "; 
sleep(1);print $red.".";sleep(1);print $white.".";sleep(1);print $black.".\n";
echo $end;

?>
